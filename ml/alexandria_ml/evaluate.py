...
def hybrid_metrics(
    hybrid: HybridRecommender, ctx: EvalContext, name: str, n_ratings: int | None = None
) -> dict[str, float]:
    ...
    def rank(u: int, _row: int) -> list[int]:
        ...
        recs = hybrid.recommend(feedback_from_ratings(ratings), k=ctx.k, exclude=ctx.seen[u], diversity=0.0)
